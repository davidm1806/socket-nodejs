...
class WebSocketServer {
   ...
   /**
    * The default TX Text Control service port (4278 for TX 29).
    * Is incremented by 1 for every major TX version.
    * @private
    */
   static get _DEFAULT_SERVICE_PORT() { return 4278; }

   /**
    * The constructor.
    * @param {http.Server} server A node.js `http.Server` object.
    * @param {{ wsPath: string, serviceAddress: string, servicePort: number }} config 
   ...
   constructor(server, config) {
      ...
      wsServer.on('connection', (ws, req) => {
         ...
         function closeTCPSocket() {
            if (socket.writable) {
               console.log('Sending "Close" command to TCP server.');
            ...
            try { socket.end(); } 
            catch (err) { console.error(`Error in net.Socket.end(): '${err}'`); }
         }
